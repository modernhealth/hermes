...
static std::vector<std::u16string_view> getSupportedNumberingSystems() {
  ...
  for (auto &supportedCalendar : supportedNumberingSystems) {
    // Return a view referencing the statically allocated string
    supportedNumberingSystemsView.push_back(
        std::u16string_view(supportedCalendar));
  }
...
std::vector<std::optional<std::u16string_view>>
getKeyLocaleDataNumberingSystems() {
  std::vector<std::optional<std::u16string_view>> numberingSystems{
      std::nullopt};

  for (auto nu : getSupportedNumberingSystems()) {
    numberingSystems.push_back(nu);
  }
...
std::vector<std::optional<std::u16string_view>> getKeyLocaleData(
  ...
  static std::vector<std::optional<std::u16string_view>>
      numberingSystemOptions = getKeyLocaleDataNumberingSystems();
